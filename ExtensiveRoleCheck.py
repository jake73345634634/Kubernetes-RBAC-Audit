...
import argparse
import logging
from colorama import init as init_coloarma, Fore
...
class ExtensiveRolesChecker(object):
    ...
    def add_warning(self, filtered_name, issue):
        self._role.warning(f"{Fore.GREEN}{filtered_name}{Fore.RED} {issue}")
        self.add_result(filtered_name, issue)
    ...
    def get_read_secrets(self, rule, role_name):
        ...
        if "secrets" in rule["resources"] and any([sign for sign in verbs if sign in rule["verbs"]]):
            ...
            if filtered_name:
                issue = "has permission to list secrets"

                self.add_warning(filtered_name, issue)
    ...
    def get_read_configmaps(self, rule, role_name):
        ...
        if "configmaps" in rule["resources"] and any([sign for sign in verbs if sign in rule["verbs"]]):
            ...
            if filtered_name:
                issue = "has permission to list configmaps"
    ...
    def clusteradmin_role(self, rule, role_name):
        if "*" in rule["resources"] and "*" in rule["verbs"]:
            ...
            if filtered_name:
                issue = "has Admin-Cluster permission"
    ...
    def any_verb(self, rule, role_name):
        ...
        if "*" in rule["verbs"]:
            ...
            if filtered_name:
                issue = f"has permission to access {found_sign[0]} with any verb"
    ...
    def any_resources(self, rule, role_name):
        ...
        if "*" in rule["resources"]:
            ...
            if filtered_name:
                issue = f"has permission to use {found_sign[0]} on any resource"
    ...
    def high_risk_roles(self, rule, role_name):
        ...
        if filtered_name:
            issue = f"has permission to {found_actions[0]} {found_attribute[0]}"
    ...
    def role_and_roleBindings(self, rule, role_name):
        ...
        if "create" in rule["verbs"]:
            ...
            if filtered_name:
                issue = f"has permission to create {found_attribute[0]}"
    ...
    def create_pods(self, rule, role_name):
        if "pods" in rule["resources"] and "create" in rule["verbs"]:
            ...
            if filtered_name:
                issue = "has permission to create pods"
    ...
    def pods_exec(self, rule, role_name):
        if "pods/exec" in rule["resources"] and "create" in rule["verbs"]:
            ...
            if filtered_name:
                issue = "has permission to use pod exec"
    ...
    def pods_attach(self, rule, role_name):
        if "pods/attach" in rule["resources"] and "create" in rule["verbs"]:
            ...
            if filtered_name:
                issue = "has permission to attach pods"
...
class RoleBindingChecker(object):
    ...
    def print_rolebinding_results(self, sub, role_name, bind_kind):
        print(f"{Fore.YELLOW}[{bind_kind}]{Fore.WHITE} {Fore.GREEN}{role_name}{Fore.RED} is bound to the {sub['kind']}: {sub['name']}")
